...
init 默认装两个 hook，把跨 session 同步和规则维护自动化：
...
- 脚本 `.claude/hooks/session-brief.sh` 和 `.claude/hooks/turn-reflect.sh`
- 启用/关闭：`.claude/settings.local.json` 的 `hooks.SessionStart` / `hooks.Stop` 段，关某个 hook 删对应子段
- 调 turn-reflect 阈值：改脚本顶部 `JOURNAL_EVERY` / `DISTILL_EVERY` / `REVIEW_EVERY`，关某一级设大值（如 `999999`）
...
└── assets/               # init 时拷的骨架
    ...
    ├── lessons/
    ├── workspace/
    ├── presets/          # 按项目类型选装的预设规则（不在 init 默认拷贝，走 init 对话按需装）
    │   ├── README.md
    │   └── coding-general.md
    └── .claude/
