...
**写入项目名和描述**（用 Edit 工具直接写，不用 shell 命令）：assets/ 里的模板文件是**范例**，用来理解"合格的 CLAUDE.md / journal.md / README.md 长什么样"。init 时 Claude 根据对话收集的事实**生成**内容，不拷贝模板、不替换占位符 —— 生成的内容天然包含项目名、描述、真实事实。
...
**配 autoMemoryDirectory**（用 Edit 工具直接写，不用 shell 命令）：

```bash
cp .claude/settings.local.json.example .claude/settings.local.json
```

然后 Edit `.claude/settings.local.json`，把 `autoMemoryDirectory` 的值替换为 `$TARGET_PATH/.claude/memory` 的绝对路径。
...
- .claude/hooks/turn-reflect.js + session-brief.js 默认启用（跨 session 同步 + 每 5/10/30 轮三级维护提醒）
...
**hooks / autoMemoryDirectory 可用性**：这一维度的本质是"hook 层在这个项目能不能跑起来"，而不只是"已有配置对不对"。可用需要三件事同时成立：（a）`.claude/hooks/session-brief.js` 和 `turn-reflect.js` 存在；（b）`.claude/settings.local.json` 里 `hooks.SessionStart` / `hooks.Stop` 段的 command 指向这两个脚本；（c）`autoMemoryDirectory` 是指向真实存在目录的绝对路径。三者任一缺失 hook 就不会触发。Read `.claude/settings.local.json`（如果存在）+ Bash 列 hooks 目录 + 验证 autoMemoryDirectory 路径。**缺失时的建议统一是"从 `assets/.claude/hooks/` 拷 session-brief.js + turn-reflect.js 到项目 `.claude/hooks/`（Node.js 脚本无需 chmod）+ 从 `assets/.claude/settings.local.json.example` 初始化 settings.local.json + Edit 替换 autoMemoryDirectory 占位为项目绝对路径"** —— 已有项目没跑过 init 时 hook 本来就没装，这维度要把"补装"当正常建议给出，不是当特殊情况处理。
...
**🔴 和 🟡 的每一项都必须一一进入建议行动，不得遗漏。** 总结段的"建议行动"列表必须覆盖所有标为 🔴 / 🟡 的维度 —— 报告里发现了问题但行动里没有，等于告诉用户"有问题但不用管"，是无效诊断。🔴 排优先，🟡 跟在后面，🟢 不需要行动条目。hooks/autoMemoryDirectory 缺失属于典型 🔴，对应行动必须给出补装步骤：① 从 `assets/.claude/hooks/` 拷 session-brief.js + turn-reflect.js 到项目 `.claude/hooks/`；② 从 `assets/.claude/settings.local.json.example` 初始化 settings.local.json；③ Edit 替换 `autoMemoryDirectory` 占位为项目绝对路径。
...
- 第二类（从 assets/ 补文件）：Bash `cp` 对应文件到目标项目（hook 脚本拷到 `.claude/hooks/`、preset 拷到 `.claude/rules/`、settings.local.json.example 拷成 settings.local.json 并用 Edit 工具替换 autoMemoryDirectory 占位为项目绝对路径）→ Read 验证文件到位 → 汇报
