...
description: >
  ...
  按报告逐条改造项目。用户说新建项目、项目骨架、项目健康检查、规不规范、按标准改项目、应用审查建议时都要触发，
  即使只是随口说"建个项目"或"扫一下这项目"。项目缺少 CLAUDE.md 或目录结构混乱时也主动建议使用。
user-invocable: true
...
用户说"建 / 初始化 / 新项目 / 从模板 / init"走 init；说"审查 / 扫 / 规不规范 / 健康检查 / audit"走 audit；说"apply / 按建议改 / 接受报告 / 应用改动"走 apply。拿不准时直接问用户 "你是想建新项目、审视已有项目，还是按上次 audit 报告改项目？"，不要猜 —— 这三个场景做的事完全不同，猜错代价大（init 新建 vs audit 只读 vs apply 动文件）。
...
- .claude/hooks/turn-reflect.sh 默认启用（每 5 轮 journal、每 10 轮蒸馏）
...
3. 不要 turn-reflect hook 就删 .claude/settings.local.json 里的 hooks 段
...
**hooks / autoMemoryDirectory 配置**：Read `.claude/settings.local.json`（如果存在）。判断 autoMemoryDirectory 是绝对路径吗、目录存在吗、hooks 引用的 `.sh` 脚本都存在且 `+x`。输出配置链断点。
...
**target_path**（可选，默认 `$(pwd)`）、上一次 audit 报告路径或内容（用户粘进来或指向项目根 `audit-report-YYYY-MM-DD.md`）。
...
**第二步：按 references 标准逐条执行**。每条改动：

- Read 对应 `references/*.md`（改 CLAUDE.md 前 Read `claudemd.md`，改 memory 前 Read `memory.md`，等等）—— 读标准不凭记忆
- 做改动（Edit / Write / Bash）
- 改完立即验证（Read 改后文件 / Glob / 看行数）
- 汇报本条完成，等用户确认再推进下一条
...
**audit 只读**。禁止用 Write / Edit / Bash 改任何文件。audit 的价值是第三方视角诊断，自动改破坏这个定位。
